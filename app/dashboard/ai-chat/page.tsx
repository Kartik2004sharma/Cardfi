...
// Story Protocol functionality removed for CardFi
...
interface SpeechRecognition extends EventTarget {
  ...
  onresult: ((this: SpeechRecognitionEvent, ev: Event) => any) | null
...
interface ImageGenerationModal {
  isOpen: boolean
...
export default function EnhancedAIChatPage() {
  ...
  const [imageGenModal, setImageGenModal] = useState<ImageGenerationModal>({
    isOpen: false,
    isGenerating: false,
    prompt: "",
  })
  ...
  // Remove old wallet connection function
  useEffect(() => {
    // No longer needed with wagmi
  }, [])
        ...
        try {
          ...
          */
          ...
          updateMessage(processingId, {
            content: `🎉 DeFi Strategy Deployed Successfully!`,
            htmlContent: `
              <div class="space-y-6 p-6">
                <div class="text-center">
                  ...
                  <h2 class="text-2xl font-bold text-green-400 mb-2">🎉 DeFi Strategy Deployed!</h2>
                  <p class="text-green-300">Your yield generation strategy is now active!</p>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div class="p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                    <h3 class="text-blue-300 font-semibold mb-3">📋 Strategy Details</h3>
                    <div class="space-y-2 text-sm">
                      <div><span class="text-white/60">Strategy:</span> <span class="text-white">${data.title || 'USDC Yield Strategy'}</span></div>
                      <div><span class="text-white/60">Expected APY:</span> <span class="text-white">12.5%</span></div>
                      <div><span class="text-white/60">Risk Level:</span> <span class="text-white">Medium</span></div>
                      <div><span class="text-white/60">Protocol:</span> <span class="text-white">Aave</span></div>
                    </div>
                  </div>
                  ...
                  <div class="p-4 bg-purple-500/10 border border-purple-500/20 rounded-lg">
                    ...
                    <div class="space-y-2 text-sm">
                      <div><span class="text-white/60">Chain:</span> <span class="text-white">Polygon</span></div>
                      <div><span class="text-white/60">Status:</span> <span class="text-white">Active</span></div>
                      <div><span class="text-white/60">TX Hash:</span> <span class="text-white font-mono">0xabc123...def456</span></div>
                    </div>
                  </div>
                </div>
                ...
                <div class="p-3 bg-blue-500/5 border border-blue-500/10 rounded-lg">
                  <p class="text-blue-300 text-sm">
                    <span class="font-semibold">Note:</span> Your strategy has been deployed and is actively earning yield. Monitor your dashboard for real-time performance.
                  </p>
                </div>

                <div class="flex flex-wrap gap-3 justify-center">
                  <a href="#" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium">
                    View Dashboard
                  </a>
                  <a href="#" class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-sm font-medium">
                    Strategy Details
                  </a>
                  <a href="#" class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm font-medium">
                    Portfolio
                  </a>
                </div>
              </div>
  ...
  const generateImageWithGemini = async (prompt: string) => {
    try {
      setImageGenModal((prev) => ({ ...prev, isGenerating: true }))
      ...
      // If generating for asset creation, convert base64 to file and continue flow
      if (imageGenModal.forAssetCreation) {
        ...
        // Close modal and continue asset creation
        setImageGenModal((prev) => ({ ...prev, isOpen: false, forAssetCreation: false }))
        handleCreateAssetFlow("Generated image added")
        return
      }

      // Also add to chat messages if not for asset creation
      addMessage("assistant", `🎨 **Image Generated Successfully!**`, {
        htmlContent: `
          <div class="space-y-4 p-4">
            <div class="p-4 bg-green-500/10 border border-green-500/20 rounded-lg">
              <div class="flex items-center gap-2 mb-2">
                ...
                <span class="text-green-300 font-semibold">Image Generated</span>
              </div>
              <p class="text-white/80 mb-3">Prompt: "${prompt}"</p>
              <div class="bg-black/50 rounded-lg p-2">
                <img src="data:image/png;base64,${data.imageBase64}" alt="Generated image" class="w-full max-w-md mx-auto rounded-lg" />
              </div>
            </div>
          </div>
        `,
      ...
      setImageGenModal((prev) => ({ ...prev, isGenerating: false }))

      addMessage("assistant", "❌ **Image generation failed**", {
        htmlContent: `
          <div class="space-y-4 p-4">
            <div class="p-4 bg-red-500/10 border border-red-500/20 rounded-lg">
              <div class="flex items-center gap-2 mb-2">
                ...
                <span class="text-red-400 font-semibold">Generation Failed</span>
              </div>
              <p class="text-red-300">${error.message}</p>
            </div>
          </div>
  ...
  const downloadGeneratedImage = () => {
    if (!imageGenModal.generatedImage) return

    const link = document.createElement("a")
    link.href = `data:image/png;base64,${imageGenModal.generatedImage.base64}`
    ...
    if (message.inputType === "file") {
      return (
        <div className="mt-4 p-4 bg-white/5 rounded-lg border border-white/10">
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            <Button
              onClick={() => {
                ...
                // Set a flag to know we're generating for asset creation
                setImageGenModal((prev) => ({ ...prev, forAssetCreation: true }))
              }}
          ...
          <p className="text-xs text-gray-400 mt-2">💡 Tip: You can upload your own image or generate one using AI</p>
        </div>
      )
    }
  ...
  const handleSendMessage = async () => {
    ...
    if (currentInput.toLowerCase().includes("generate image") || currentInput.toLowerCase().includes("create image")) {
      openImageGenerationModal()
      setLoading(false)
      return
    }
    ...
    try {
      ...
      console.log("Starting enhanced agent processing with wallet:", address)
      const response: any = await enhancedIntelligentAgent(currentInput, api, address)
  ...
  const querySuggestions: QuerySuggestion[] = [
    ...
    {
      text: "Show highest earning assets",
      category: "Assets",
      icon: "💎",
      description: "Most valuable assets by royalties",
    },
    { text: "Generate Image", category: "Assets", icon: "🖼️", description: "Create AI-generated images" },
    ...
    // Analytics
    {
      ...
      category: "Analytics",
      icon: "💰",
      description: "Today's royalty payments in IP tokens",
    },
    {
    ...
    {
      text: "Show Latest 5 Transactions",
      category: "Analytics",
      icon: "📊",
      description: "Complete platform statistics",
    },
    {
      text: "Show Royalties of this asset {Asset_address}",
      category: "Analytics",
      icon: "📊",
      description: "Complete platform statistics",
    },
    {
      text: "Show Minting Fees of this asset {Asset_address}",
      category: "Analytics",
      icon: "📊",
      description: "Complete platform statistics",
    },
  ...
  return (
    <div className="flex flex-col h-full w-full bg-background text-white overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            <Button
              ...
              variant="outline"
              size="sm"
              className="border-gray-700 text-gray-400 hover:bg-gray-800 hover:text-white flex items-center gap-2"
            >
              <ImageIcon className="h-4 w-4" />
              Generate Image
            </Button>
            ...
            <div className={`flex max-w-[95%] ${message.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
              <div
                className={`rounded-full h-10 w-10 flex items-center justify-center shrink-0 ${
                  message.role === "user" ? "bg-white ml-3" : "bg-black border border-gray-700 mr-3"
                }`}
              >
                {message.role === "user" ? (
              ...
              <div
                className={`py-4 px-6 rounded-lg border ${
                  message.role === "user" ? "bg-white text-black border-gray-300" : "bg-black border-gray-700"
                }`}
              >
                {message.htmlContent ? (
                  <div
        ...
        <div className="relative">
          <div className="flex items-end gap-4 p-4 border border-gray-700 rounded-lg bg-black focus-within:border-white focus-within:bg-gray-900 transition-all duration-300">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {voiceRecognition.isListening ? <MicOff className="h-4 w-4" /> : <Mic className="h-4 w-4" />}
              </Button>
          ...
          {voiceRecognition.isSupported && !voiceRecognition.isListening && (
            <div className="mt-2 text-center">
              <p className="text-xs text-gray-500">
                💡 Tip: Click the <Mic className="inline h-3 w-3 mx-1" /> microphone or press{" "}
                <kbd className="px-1 py-0.5 bg-gray-800 border border-gray-600 rounded text-xs">Ctrl+Shift+M</kbd> for
                voice input
              </p>
            </div>
          )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {suggestions.map((suggestion, idx) => (
                    <button
                      key={idx}
                      onClick={() => {
                        if (suggestion.text === "Generate Image") {
                          openImageGenerationModal()
                        } else {
      ...
      <Dialog
        open={imageGenModal.isOpen}
        onOpenChange={(open) => setImageGenModal((prev) => ({ ...prev, isOpen: open }))}
      >
        <DialogContent className="bg-black border-gray-700 text-white max-w-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-white">
              <ImageIcon className="h-5 w-5" />
              {imageGenModal.forAssetCreation ? "Generate Image for IP Asset" : "AI Image Generation"}
            </DialogTitle>
          </DialogHeader>

          <div className="space-y-6 h-[88vh]">
            {imageGenModal.forAssetCreation && (
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3 overflow-y-scroll ">
                <div className="flex items-center gap-2 mb-2">
                  <Info className="h-4 w-4 text-blue-400" />
                  <span className="text-blue-300 font-semibold">Creating Asset Image</span>
                </div>
                <p className="text-blue-200 text-sm">
                  This image will be used for your IP asset. Once generated, it will be automatically added to your
                  asset creation.
                </p>
              </div>
            )}
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-300">
                {imageGenModal.forAssetCreation
                  ? "Describe the image for your IP asset:"
                  : "Describe the image you want to generate:"}
              </label>
              <Textarea
                value={imageGenModal.prompt}
                onChange={(e) => setImageGenModal((prev) => ({ ...prev, prompt: e.target.value }))}
                placeholder={
                  imageGenModal.forAssetCreation
                    ? "e.g., A professional logo for my brand, minimalist design with blue and white colors..."
                    : "e.g., A futuristic cityscape at sunset with flying cars and neon lights..."
                }
                className="bg-gray-900 border-gray-700 text-white placeholder:text-gray-500 min-h-[100px]"
                disabled={imageGenModal.isGenerating}
              />
            </div>

            {/* Generated Image Display */}
            {imageGenModal.generatedImage && (
              <div className="space-y-3">
                ...
                <div className="bg-gray-900 rounded-lg p-4">
                  <img
                    src={`data:image/png;base64,${imageGenModal.generatedImage.base64}`}
                    alt="Generated image"
                    className="w-full max-w-md mx-auto rounded-lg"
                  />
                  <p className="text-xs text-gray-500 mt-2 text-center">
                    Prompt: {imageGenModal.generatedImage.prompt}
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="flex justify-end gap-3">
              <Button
                onClick={() => setImageGenModal((prev) => ({ ...prev, isOpen: false, forAssetCreation: false }))}
                variant="outline"
              ...
              <Button
                onClick={() => generateImageWithGemini(imageGenModal.prompt)}
                disabled={!imageGenModal.prompt.trim() || imageGenModal.isGenerating}
                className="bg-white hover:bg-gray-200 text-black flex items-center gap-2"
              >
                {imageGenModal.isGenerating ? (
                  <>
                  ...
                  <>
                    <ImageIcon className="h-4 w-4" />
                    {imageGenModal.forAssetCreation ? "Generate & Use" : "Generate Image"}
                  </>
                )}
              </Button>
            </div>
